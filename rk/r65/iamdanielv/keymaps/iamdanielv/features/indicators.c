...
bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    ...
    if (IS_LAYER_ON(KBCTL_LYR)) {
        ...
        // layer lock key
        set_layer_lock_color_indicator(KBCTL_LYR, LEFT_WIN_KI, (rgb_t){RGB_WHITE}, led_min, led_max);
    }
    ...
    if (IS_LAYER_ON(MEDIA_LYR)) {
        ...
        // layer lock key
        set_layer_lock_color_indicator(MEDIA_LYR, LEFT_WIN_KI, (rgb_t){RGB_MEDIA_KEYS}, led_min, led_max);
    }
